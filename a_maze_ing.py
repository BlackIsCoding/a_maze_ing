    ...
    if config:
        ...
        try:
            ...
            while True:
                ...
                while max_y < (
                        config["HEIGHT"] * 2) + 6 or max_x < (config['WIDTH']
                                                              * 4):
                    stdscr.clear()
                    ...
                    if max_y > (config["HEIGHT"] * 2 + 6):
                        if max_x > (config["WIDTH"] * 4):
                            break

                for i, item in enumerate(menu):
                    if i == current:
                        stdscr.addstr(
                            (max_y // 2) + i, (max_x // 2) - len(item) // 2,
                            item, curses.color_pair(4))
                    else:
                        stdscr.addstr((max_y // 2) + i,
                                      (max_x // 2) - len(item) // 2, item)
                ...
                if key in [curses.KEY_ENTER, 10, 13]:
                    if menu[current] == "START MAZE":
                        ...
                        while True:
                            ...
                            if path:
                                draw_maze(stdscr, path, output["output"],
                                          entry, exit, show_path, rotate_color,
                                          output["blocked"])
                            else:
                        ...
                        draw_maze_game(stdscr, output["output"],
                                       entry, exit, 6, output["blocked"])
                        stdscr.refresh()
                        time.sleep(0.05)
